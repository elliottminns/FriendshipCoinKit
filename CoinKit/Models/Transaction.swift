...
public struct Transaction {
  ...
  init(version: UInt32, locktime: UInt32, inputs: [Input], outputs: [Output]) {
  ...
  fileprivate func byteLength(allowWitness: Bool) -> UInt {
  ...
  func hashForSignature(index: Int, prevOutScript: Data, hashType: UInt8) throws -> Data {
    ...
    let tx = Transaction(version: self.version, locktime: self.locktime, inputs: ins, outputs: outs)
    var buffer = Data()

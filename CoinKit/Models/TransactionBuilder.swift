...
extension Transaction {
  ...
  public class Builder {
    ...
    var ins: [Transaction.Input.Expanded] = []
    ...
    init(transaction: Transaction, network: Network) {
    ...
    func add(transaction: Transaction) throws -> UInt {
    ...
    func add(transaction: String) throws -> UInt {
      ...
      let input = Transaction.Input(hash: data, index: UInt32(index))
    ...
    @discardableResult func add(input: Transaction.Input) throws -> UInt {
    ...
    func add(output script: Data, amount: UInt64) {
      let output = Transaction.Output(amount: amount, script: script)
      self.add(output: output)
    }
    
    func add(output: Transaction.Output) {
    ...
    func build() throws -> Transaction {
      ...
      let inputs = try self.inputs.enumerated().map { obj -> Transaction.Input in
        let built = try build(input: obj.offset)
        let input = obj.element
        return Transaction.Input(hash: input.hash, index: input.index, script: built.script, sequence: input.sequence, witness: input.witness)
      }
      return Transaction(version: self.version ?? 1, locktime: locktime ?? 0, inputs: inputs, outputs: outputs)
    }
    ...
    func sign(vin: Int, signable: Signable) throws {
      ...
      let hashType = Transaction.Constant.sighashAll
      ...
      let transaction = Transaction(version: version ?? 1, locktime: locktime ?? 0, inputs: inputs, outputs: outputs)
